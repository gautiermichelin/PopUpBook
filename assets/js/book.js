...
$document.ready(function () {
	...
	if (has3d) {
		...
		if (hasTouch) {
			hideLocationBar();
			$dragNotice.html('Tournez <br />les pages!');
			$dragNotice.css({
				'width': '220px'
			});
		}
        ...
        $(".qqch").each(function() {
            if( $(this).data('behind') != undefined) {
                var s = "." + $(this).data('behind').toString();
                $(this).attr("title", $(s).data('title').toString());
            }
        });
...
function updateDrag(e) {
	...
	if ((curPageIndex == 0 && adjustedPer > 0) || (curPageIndex == pageNumber && adjustedPer < 0)) {
		stopDrag();
		curPer = adjustedPer = 0;
		return;
	}
    ...
    } else if (curPageIndex == (pageNumber - 1) && adjustedPer < 0) {
        rotateTransform = 'rotateX(' + (90 - (61 * absPer)) + 'deg) rotateZ(' + (-8 * absPer) + 'deg)';
        translateTransform = 'translate3d(' + (300 * absPer) + 'px, ' + (-120 * absPer) + 'px, ' + (100 * absPer) + 'px)';
    } else if (curPageIndex == pageNumber) {
...
function onHotspotSelected(e) {
	...
	if (zoomedIn) {
		...
		var translateTransform;
		if ($spread.hasClass('pilier-rouge-wrapper')) {
			translateTransform = 'translate3d(-330px, -150px, 400px)';
		} else if ($spread.hasClass('maison-des-deux-amis-wrapper')) {
			translateTransform = 'translate3d(-330px, -150px, 400px)';
		} else if ($spread.hasClass('palais-des-comtes-du-maine-wrapper')) {
            translateTransform = 'translate3d(-330px, -150px, 400px)';
		} else if ($spread.hasClass('muraille-du-mans-wrapper')) {
            translateTransform = 'translate3d(-330px, -150px, 400px)';
        }
...
function Popup(graphic, depth, event) {
	...
	if(curPageIndex == pageNumber) {
        var zRot = graphic.parents('.noise').hasClass('page-left') ? 15 : -15;
	}
...
$(".qqch").mouseenter(function() {
    if($(this).data('behind') != '') {
        var s = "." + $(this).data('behind').toString();
        $("div.popup").css("filter", "grayscale(100%)");
        $(s).css("filter", "grayscale(0%)");
        $(this).tooltip({
            track:true
        });
    }
...
.mouseleave(function() {
    if($(this).data('behind') != "") {
        var s = "." + $(this).data('behind').toString();
        $("div.popup").css("filter", "grayscale(0%)");
    }
});
